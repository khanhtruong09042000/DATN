...
export default function Table_HP() {
  ...
  const handleOpen = (id) =>
    {
      ...
      fetch(`https://datn-production.up.railway.app/subjects/${id}`)
  ...
  return (
    <Paper sx={{ width: '100%', overflow: 'hidden' , margin: "30px 0px 0px 30px"}}>
        ...
        <Table stickyHeader aria-label="sticky table">
          ...
          <TableBody>
            {HP
              ...
              .map((row, index) => {
                return (
                  <TableRow hover role="checkbox" tabIndex={-1} key={row.code} onClick = {()=>handleOpen(row.MA_HP)} style={{cursor: 'pointer'}}>
      ...
      />
      <Popup_HP data = {open} is={setOpen} SV = {CT}/>
    </Paper>
  );
}
